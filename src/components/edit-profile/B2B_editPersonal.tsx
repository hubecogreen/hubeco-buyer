...
const B2B_editPersonal: React.FC<ProfileFormProps> = ({
  imgSrc,
  // mobileNumber,
  nextStep
}) => {
  ...
  // const otpValues = watch1(['digit1', 'digit2', 'digit3', 'digit4', 'digit5', 'digit6']);
 
  const [intialNumber,setIntialNumber]= useState<string>(buyerInfo?.number.replace(/^\+91/, ''));
  ...
  const VerifyMobile=async(data:any)=>{
   // // console.log('sdvsvs',data)
    setIsLoading2(true)

        const payload = {
          otp: Number(`${data.digit1}${data.digit2}${data.digit3}${data.digit4}${data.digit5}${data.digit6}`),
          otpToken: getCookie('updateMobileOtpToken')
        };
  
    
        try{
          const result =(await callApi(changeType=='mobile'?getEndpoint.default.UPDATEMOBILE:getEndpoint.default.UPDATEEMAIL,'POST',payload))
        ...
        if(result.data==null)
          {
            ...
            toast.success(changeType=='mobile'?"Mobile Number Updated Successfully": "Email Updated Successfully",{iconTheme: {
              primary: '#439787',
              secondary: '#FFFAEE',
            }});
            setOtpValue('')
  ...
  const handleSendOtpApiError = async (err: any,from:any) => {
    const result = err && err.response
      ...
      if(result?.data?.intent == "ERROR" && result?.data?.message == "Invalid authentication token")
        {
          await refreshTokens();
          if(from=='fromMobile')
            {
  ...
  const sendOTP=async(from:any)=>{
    ...
    : '')
        ...
        try{
          const result =(await callApi(getEndpoint.default.SENDUPDATEPHONEOTP,'POST',payload)) as any;
       ...
       // // console.log('resultUpdatePhoenOtp',result)
        if(result.data==null)
          {
            ...
            toast.success("OTP Sent to New Mobile Number",{iconTheme: {
              primary: '#439787',
              secondary: '#FFFAEE',
            }});
        ...
        try{
          const result =(await callApi(getEndpoint.default.SENDUPDATEEMAILOTP,'POST',payload)) as any;
        ...
        if(result.data==null)
          {
            ...
            toast.success("OTP Sent to New Email",{iconTheme: {
              primary: '#439787',
              secondary: '#FFFAEE',
            }});
            setShowVerifyPopup(true)
      ...
      if(result.data==null)
        {
         ...
         // // console.log('lggedin',result.data)
          if(result?.data?.intent=="EMAIL_LINK_SENT")
            {
              // setIsOpen(true)
              toast.success("Verfiy Email link sent to the registered email.",{iconTheme: {
                primary: '#439787',
                secondary: '#FFFAEE',
              }})
...
reset1()
  ...
  const handleInputChange = (onChange: (value: string) => void, value: string, index: number,field:any) => {
    onChange(value);
   

    if(document.querySelectorAll('.otpfield').length==6)
      {
        clearErrors1();
      }
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex flex-grow flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <label
                className="cursor-pointer py-2 px-4 rounded inline-block bg-[#0098862E] text-[#009886] transition-colors duration-300 ease-in-out"
              >

              {buyerInfo?.displayImage || file   ? 'Change Profile Picture' : 'Upload Profile Picture'}
...
{buyerInfo?.emailVerified==false && !errors?.email ? <span className="text-red py-2 mt-1 text-left text-[14px]">* Email is not verified.<span className="text-primary text-[14px] cursor-pointer" onClick={onClickVerify}> Click Here</span> to resend verify email link.</span>:<></>}
</div>
        </div>
        <div className="w-full sm:w-1/2 p-2 sm:p-5">
        <div className="flex  justify-between items-center">
...
{editMobile ?
    ...
    {/*   <DialogClose className='absolute right-4 top-4'>
        ...
        <form
              ...
              onSubmit={handleSubmit1(VerifyMobile)}
            >
              <div className=" ">
         ...
         <PinInput>
            {['digit1', 'digit2', 'digit3', 'digit4', 'digit5', 'digit6'].map((field, index) => (
              <Controller
                key={field}
                name={field}
                control={control1}
          ...
          <CustomButton type="submit"  title="Submit" loading={isLoading2} className="w-28 md:28 bg-secondary hover:bg-primary text-white"/> 
          </DialogFooter>
          ...
          <button
            type="button"
            className="text-secondary text-center cursor-pointer font-semibold text-sm ml-3 md:text-sm"
            onClick={resendOTP}
          >
            Resend
          </button>
...
export default B2B_editPersonal;
