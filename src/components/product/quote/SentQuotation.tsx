 ...
 async function handleQuoteAction() {
  ...
  <thead className="bg-secondaryBg  text-left ">
    <tr>
      ...
      <th className="p-4 whitespace-nowrap text-left">Offered Quantity</th>
      <th className="p-4 whitespace-nowrap text-left">Tax Percentage</th>
      <th className="p-4 whitespace-nowrap text-left">Delivery Terms</th>
      <th className="p-4 whitespace-nowrap text-left">Other Terms and Conditions</th>
                ...
                >
                  {elem.cgst !== -1 && (
                    <p className=" flex justify-start text-sm gap-3">
                      <span>CGST:</span>
                      <span>{elem?.cgst ? `${elem.cgst}%` : "N/A"}</span>
                    </p>
                  )}
                  {elem.sgst !== -1 && (
                    <p className=" flex justify-start text-sm gap-3">
                      <span>SGST:</span>
                      <span>{elem?.sgst ? `${elem.sgst}%` : "N/A"}</span>
                    </p>
                  )}
                  {elem?.igst !== -1 && (
                    <p className=" flex justify-start text-sm gap-3">
                      <span>IGST:</span>
                      <span>{elem?.igst ? `${elem.igst}%` : "N/A"}</span>
                    </p>
                  )}
                </div>
              ))
        ...
        <td className="p-4 min-w-[220px]">
          <div className="flex flex-col space-y-2">
            <div className="flex justify-between">
              <span className="text-gray-600 pr-3">Taxable Amount:</span>
            ...
            <div className="flex justify-between">
              <span className="text-gray-600">
                GST ({quote.products[0].cgst === -1 ? 'IGST' : 'CGST + SGST'}):
              </span>
              <span className="font-medium">
              <span className="text-xl font-normal font-mono">₹</span>
                ...
                {Array.isArray(quote.products) &&
                  formatCurrencyInIndianStyle(quote.products.reduce(
            ...
            <div className="flex justify-between">
              ...
              <span className="text-xl font-normal font-mono">₹</span>
                ...
                {Array.isArray(quote.products) &&
                  formatCurrencyInIndianStyle(quote.products
                    .reduce((total: any, product: { shippingCost: any }) => total + product.shippingCost, 0)
                  )}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="text-xl font-normal font-mono">₹</span>
                {Array.isArray(quote.products) &&
                  formatCurrencyInIndianStyle(quote.products
                    .reduce((total: any, product: { otherCost: any }) => total + product.otherCost, 0)
                  )}
              </span>
            </div>
            ...
            {Array.isArray(quote.products) && quote.products.some(elem => elem.roundValue !== 0) && (
              <div className="justify-between items-center flex">
                <div className="inline text-xs font-medium">Amount rounded off to</div>
                <div className="flex flex-row">
                  <span className="font-mono mr-0 inline">₹</span>
                  <span className="inline-flex flex-col text-xs items-start w-full pt-[2px]">
                    {
                      quote.products
                        .reduce((total: any, product: { roundValue: any }) => Number(total) + Number(product.roundValue), 0)
                        .toFixed(2)
                    }
                  </span>
                </div>
