...
const B2CPersonal: React.FC<ProfileFormProps> = ({ imgSrc, nextStep }) => {
  const otpToken = getCookie("otpToken");
  const { callApi } = useApi();
  const { personalInfo, setPersonalInfo } = useB2CContext();
  ...
  const {
    ...
    defaultValues: {
      ...
      mobile: mobileNumber
        ? mobileNumber
        : buyerDetails?.number
        ? buyerDetails?.number.replace("+91", "")
        : "",
  ...
  // // console.log('personalInfo',mobileNumber)
  ...
  useEffect(() => {
    if (personalInfo) {
      ...
      setValue("privacyPolicy", personalInfo?.privacyPolicy);
      setValue("tandc", personalInfo?.tandc);
      // clearErrors()
    }
  }, [personalInfo]);
    ...
    if (getCookie("userRegistered") == "true") {
      ...
      try {
        ...
        if (result.data == null) {
          ...
          // router.push('/welcome')
          setCookie("userRegistered", "true");
          setCookie("privacyPolicyCheck", "true");
          setCookie("tandcCheck", "true");
          setPersonalInfo({
            email: data.email,
            mobile: `+91${data.mobile}`,
            firstName: data.firstName,
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <Image
            ...
            src={
              file != null
                ...
                : getCookie("userRegistered") == "true"
                ? personalInfo.displayImage != ""
                  ? `${assetURL}/${personalInfo?.displayImage}`
                  : imgSrc
                : imgSrc
            }
            alt="Profile"
          ...
          <div className="flex flex-grow flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <label className="cursor-pointer py-2 px-4 rounded inline-block bg-[#0098862E] text-[#009886] transition-colors duration-300 hover:bg-[#0098863A]">
                {personalInfo?.displayImage === "" && file == null
        ...
        <AlertDialog open={isOpen}>
          ...
          <AlertDialogContent>
            <AlertDialogHeader className="mb-[10px]">
              <AlertDialogTitle className="mb-[5px]">
                Are you sure you want to continue?
              </AlertDialogTitle>
              <AlertDialogDescription className="mb-[15px]">
                Please verify your details before submitting. Once submitted,
