...
const B2CSecurity: React.FC<SecurityFormProps> = ({
  ...
  const {setShippingInfo,setPersonalInfo,setCompanyInfo,setSecurityInfo,setb2bPersonalInfo,setb2bShippingInfo,securityInfo}=useB2CContext()
  const {callApi}=useApi()
  const router =useRouter()

  useEffect(() => {
    if (securityInfo) {
      setValue("newPassword", securityInfo?.newPassword);
      setValue("confirmNewPassword", securityInfo?.confirmNewPassword);
    }
  ...
  const handleSecuritySubmit = async (data: any) => {
   // // console.log("Security Data:", data);
    ...
    setIsLoading(true)
    if(securityInfo && securityInfo?.password && securityInfo?.confirmPassword)
      {
        // nextStep()
    ...
    // Webservices.callPatchApi(getEndpoint.default.SETPASSWORD, payload, token)
    //   .then((result) => {
    //    // // console.log("Form Submission", result.data);

    //     if (result.status === 200) {
    //       toast.success("Security Info Submitted Successfully");
    //       setIsLoading(false)
    //       handleSubmit();
    //     } else {
...
export default B2CSecurity;
function setShippingInfo(arg0: { address: string; city: string; state: string; postCode: string; country: string; landmark: string; }) {
  throw new Error("Function not implemented.");
}

function setCompanyInfo(arg0: { companyName: string; businessType: string; gst: string; tan: string; pan: string; companyaddress: string; website: string; }) {
  throw new Error("Function not implemented.");
}

function setPersonalInfo(arg0: { email: string; mobile: string; firstName: string; lastName: string; displayImage: string; privacyPolicy: boolean; tandc: boolean; }) {
  throw new Error("Function not implemented.");
}
